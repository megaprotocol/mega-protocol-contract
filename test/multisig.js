...
const MegaCoin = artifacts.require('MegaCoin.sol');
const MegaCoinMultiSigFactory = artifacts.require('MegaCoinMultiSigWallet.sol');
const web3 = MegaCoin.web3;
const eth = web3.eth;
...
const deployMultiSigWallet = (owners, requiredConfirm, coin) => {
  return MegaCoinMultiSigFactory.new(owners, requiredConfirm, coin, "vault");
};

const deployMegaCoin = (owner,admin,vault) => {
  return MegaCoin.new("Mega", "MegaCoin", owner, admin, vault);
};
...
contract("MegaCoinMultiSigWallet", (accounts) => {
  const BIG = (v) => new theBN.BN(v)
  let coinInstance;
  let walletInstance;
  const requiredConfirmations = 2;

  let OneMegaCoinInMinunit;
  ...
  beforeEach(async () => {
    coinInstance = await deployMegaCoin(owner, admin, vault);
    assert.ok(coinInstance);

    NoOfTokensInMinunit = await coinInstance.totalSupply();
    OneMegaCoinInMinunit = await coinInstance.getOneMegaCoin();
  ...
  it("multisig wallet basic functions", async () => {
    assert.equal( await walletInstance.megaCoin(), coinInstance.address );
  ...
  it("only signers can request TX", async () => {
    // user1 cannot
    assertRevert("1-1",() => walletInstance.requestTransferInMega(user1, 1, 0, "test 1", {from: user1}));
    ...
    // owner cannot
    assertRevert("2-1",() => walletInstance.requestTransferInMega(user1, 1, 0, "test 2-1", {from: owner}));
    ...
    // admin(as signer) can
    assertNotRevert("3-1",() => walletInstance.requestTransferInMega(user1, 1, 0, "test 3-1", {from: admin}));
    ...
    // vault(as signer) can
    assertNotRevert("4-1",() => walletInstance.requestTransferInMega(user1, 1, 0, "test 4-1", {from: vault}));
    ...
    assertNotRevert("5-1",() => walletInstance.requestTransferInMega(user1, 1, 0, "test 5-1", {from: signer}));
  ...
  it("signers only methods", async () => {
    // request a TX
    walletInstance.requestTransferInMega(user1, 1, 0, "test 1", {from: signer});
  ...
  it("test transfer using multisig wallet", async () => {
    ...
    const thousandMegaInMinunit = OneMegaCoinInMinunit.mul(BIG(1000))
    coinInstance.transfer(walletInstance.address, thousandMegaInMinunit, {from:vault});
    assert.equal(await balanceOf(walletInstance.address), thousandMegaInMinunit.toString());

    // Add megacoin transfer transaction
    // example ABI generation:
    // const transferData = coinInstance.contract.transfer.getData(user1, OneMegaCoin * 1);
    //

    const MSG1 = "send 1Mega and 1DENNIS to user1";
    const trSubmit = await walletInstance.requestTransferInMega(user1, 1, 1, MSG1, {from:vault});
    ...
    assert.equal(afterBalance.toString(), OneMegaCoinInMinunit.mul(BIG(1)).add(BIG(1)).toString());
  ...
  it("Add a signer", async () => {
    ...
    assertNotRevert("signer1",() => walletInstance.requestTransferInMega(user2, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Remove a signer", async () => {
    ...
    assertRevert("signer1",() => walletInstance.requestTransferInMega(user1, 1, 0, "test addsigner-1", {from: user1}));
