...
const MegaCoinMultiSigFactory = artifacts.require('MegaCoinMultiSigWalletWithMint.sol');
...
contract("MegaCoinMultiSigWalletWithMint", (accounts) => {
  ...
  it("Add a signer", async () => {
    ...
    assertNotRevert("signer1",() => walletInstance.requestMintInMega(user1, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Remove a signer", async () => {
    ...
    assertRevert("signer1",() => walletInstance.requestMintInMega(user1, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Add and remove a minter", async () => {
      ...
      assertNotRevert("user1 can mint", () => coinInstance.mint(user1, OneMegaCoinInMinunit, {from: user1}));
      let afterBalance = await coinInstance.balanceOf(user1);

      // balance difference should be same as the minted amount
      assert.equal(afterBalance.sub(prevBalance).toString(), OneMegaCoinInMinunit.toString());
      ...
      assertRevert("user1 cannot mint after renounce", () => coinInstance.mint(user1, OneMegaCoinInMinunit, {from: user1}));
  ...
  it("test mint using multisig wallet", async () => {
        ...
        const thousandMega = BIG(1000)
        const zero = BIG(0)

        const MSG1 = "MINT 1000 Mega to user1";
        const trSubmit = await walletInstance.requestMintInMega(user1, thousandMega, zero, MSG1, {from:vault});
        ...
        assert.equal(afterBalance.toString(), thousandMegaInMinunit.toString());
