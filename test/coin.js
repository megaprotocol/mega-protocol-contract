...
/**
 * MegaCoin contract learning test
 *   deploy in before, and no deploy between it()
 */
contract("MegaCoin", (accounts) => {
  ...
  let NoOfTokensInMinunit, OneMegaCoinInMinunit, NoOfTokensInMega;
  let coin
  ...
  before(async () => {
    ...
    OneMegaCoinInMinunit = BIG(await coin.getOneMegaCoin());
    NoOfTokensInMega = NoOfTokensInMinunit.div(OneMegaCoinInMinunit)
  });
  ...
  it("coin initial values", async () => {
    ...
    assert.equal(await coin.symbol(), "MEGA");
    assert.equal(await coin.name(), "MEGACoin");
    assert.equal(await coin.decimals(), 9);
    assert.equal((await coin.totalSupply()).toString(), BIG(NOOFTOKENS).mul(OneMegaCoinInMinunit).toString());
  ...
  it("token distribution", async () => {
    ...
    await coin.transfer(admin, OneMegaCoinInMinunit, {from: vault});
    assert.equal(await balanceOf(admin), OneMegaCoinInMinunit.toString());

    // transfer 0.5 token from admin to owner
    let halfMega = OneMegaCoinInMinunit.div(BIG(2))
    await coin.transfer(owner, halfMega, {from: admin});
    assert.equal(await balanceOf(owner), halfMega.toString());
    assert.equal(await balanceOf(admin), halfMega.toString());
    assert.equal(await balanceOf(vault), (NoOfTokensInMega.sub(BIG(1)).mul(OneMegaCoinInMinunit).toString()));
  });
  ...
  it("marginal transfers", async () => {
    ...
    try {
      await coin.transfer(user2, OneMegaCoinInMinunit.mul(BIG(1)), {from: user1});
      assert.fail();
    }
    ...
    try
    {
      await coin.transfer(user2, OneMegaCoinInMinunit.mul(BIG(0)), {from: user1});
      assert.equal(await balanceOf(user1), OneMegaCoinInMinunit.mul(BIG(0)).toString());
      assert.equal(await balanceOf(user2), OneMegaCoinInMinunit.mul(BIG(0)).toString());
    }
    catch(exception) { assert.fail(); }

    // transfer 0 token from vault(balance > 0) to user1
    try {
        await coin.transfer(user1, OneMegaCoinInMinunit.mul(BIG(0)), {from: vault});
        assert.equal(await balanceOf(user1), OneMegaCoinInMinunit.mul(BIG(0)).toString());
    } catch(exception) { assert.fail(); }

    // transfer whole balance to other
    await coin.transfer(user1, OneMegaCoinInMinunit.mul(BIG(1)), {from: vault});
    assert.equal(await balanceOf(user1), OneMegaCoinInMinunit.mul(BIG(1)).toString());
    await coin.transfer(user2, OneMegaCoinInMinunit.mul(BIG(1)), {from: user1});
    assert.equal(await balanceOf(user1), OneMegaCoinInMinunit.mul(BIG(0)).toString());
    assert.equal(await balanceOf(user2), OneMegaCoinInMinunit.mul(BIG(1)).toString());
  });
  ...
  it("only admin can set reserve", async () => {
    await coin.setReserve(user1, OneMegaCoinInMinunit, {from: admin});
  ...
  it("reserve and reserveOf", async () => {
    ...
    await coin.transfer(user1, OneMegaCoinInMinunit.mul(BIG(10)), { from: vault });
    await coin.setReserve(user1, OneMegaCoinInMinunit.mul(BIG(5)), { from: admin });
    assert.equal(await reserveOf(user1), OneMegaCoinInMinunit.mul(BIG(5)).toString());
  });
  
  it("transfer after setting reservation", async () => {
    // user1 = 10 Mega
    await coin.transfer(user1, OneMegaCoinInMinunit.mul(BIG(10)), { from: vault });
    // user1 = 10 Mega with 5 Mega reserve
    await coin.setReserve(user1, OneMegaCoinInMinunit.mul(BIG(5)), { from: admin });
    // user1 = 9 Mega with 5 Mega reserve
    await coin.transfer(user2, OneMegaCoinInMinunit.mul(BIG(1)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneMegaCoinInMinunit.mul(BIG(9)).toString());

    // user1 = 6 Mega with 5 Mega reserve
    coin.transfer(user2, OneMegaCoinInMinunit.mul(BIG(3)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneMegaCoinInMinunit.mul(BIG(6)));

    try {
      let tx = await coin.transfer(user2, OneMegaCoinInMinunit.mul(BIG(2)), {from: user1}); // violating reserved amount
      assert.equal(await balanceOf(user1), OneMegaCoinInMinunit.mul(BIG(6)));
      assert.fail();
    }
    catch(exception) {
